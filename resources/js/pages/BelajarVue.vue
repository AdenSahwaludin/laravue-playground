...
const updateMessage = () => {
    state.value.message = 'Anda mengklik tombol!';
};
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="dark:bg-sidebar m-4 flex flex-col gap-4 bg-gray-100 p-4">
            <h1>{{ state.message }}</h1>
            ...
            <div class="flex w-full flex-col gap-4 md:flex-row">
                ...
                <div class="w-full">
                    <Button as="a" :href="route('belajar')" class="w-full cursor-pointer hover:cursor-pointer">Pergi ke ToDoList</Button>
                </div>
            </div>
            <p>Kamu adalah {{ props.user.name }}</p>
            <ul>
                List semua user:
                <li v-for="(user, index) in props.users" :key="user.id">{{ index + 1 }}. {{ user.name }}</li>
            </ul>
        </div>
    </AppLayout>
</template>
